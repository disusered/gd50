...
function PlayState:update(dt)
    ...
    if love.mouse.wasPressed(1) then
      ...
      -- check if mouse is over the board
        local mouseOverBoard = x > VIRTUAL_WIDTH - 272 and
                          x < VIRTUAL_WIDTH - 16 and
                          y > 16 and y < VIRTUAL_HEIGHT - 16

        -- if so, highlight tile under mouse. without the check, this would
        -- lead to being able to highlight areas outside of the board
        if mouseOverBoard then
          self.boardHighlightX = math.floor((x - (VIRTUAL_WIDTH - 272)) / 32)
          self.boardHighlightY = math.floor((y - 16) / 32)
        end
    end

		-- if we've pressed enter or click on the mouse button to select or deselect a tile...
		if love.keyboard.wasPressed("enter") or love.keyboard.wasPressed("return") or love.mouse.wasPressed(1) then
			...
			elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
				...
				if matches then
					-- if the swap is valid, we can continue playing
					...
					-- play error sound if swap not valid
					gSounds["error"]:play()

					-- swap grid positions of tiles back
					...
					-- reset highlighted tile since swap not valid
					self.highlightedTile = nil
				end
			end
		end
